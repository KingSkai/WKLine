...
static NSInteger const WK_StockChartSegmentStartTag = 2000;

//static CGFloat const WK_StockChartSegmentIndicatorViewHeight = 2;
//
//static CGFloat const WK_StockChartSegmentIndicatorViewWidth = 40;

@interface WK_StockChartSegmentView()
...
@implementation WK_StockChartSegmentView
...
- (UIView *)indicatorView
{
    if(!_indicatorView)
    {
        ...
        [titleArr enumerateObjectsUsingBlock:^(NSString*  _Nonnull title, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            btn.tag = WK_StockChartSegmentStartTag + 100 + idx;
...
- (void)setItems:(NSArray *)items
{
    ...
    {
        UIButton *btn = [self private_createButtonWithTitle:title tag:WK_StockChartSegmentStartTag+index];
        UIView *view = [UIView new];
        view.backgroundColor = [UIColor colorWithRed:52.f/255.f green:56.f/255.f blue:67/255.f alpha:1];
        [self addSubview:btn];
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex
{
    ...
    UIButton *btn = (UIButton *)[self viewWithTag:WK_StockChartSegmentStartTag + selectedIndex];
...
- (void)setSelectedBtn:(UIButton *)selectedBtn
{
    ...
    {
        if(selectedBtn.tag != WK_StockChartSegmentStartTag)
        {
            return;
        } else {
    ...
    } else if(selectedBtn.tag != WK_StockChartSegmentStartTag){
        [_selectedBtn setSelected:NO];
        [selectedBtn setSelected:YES];
        _selectedBtn = selectedBtn;
    }
    
    _selectedIndex = selectedBtn.tag - WK_StockChartSegmentStartTag;
...
- (void)event_segmentButtonClicked:(UIButton *)btn
{
    ...
    if(btn.tag == WK_StockChartSegmentStartTag)
    {
        return;
    }
    
    if(self.delegate && [self.delegate respondsToSelector:@selector(WK_StockChartSegmentView:clickSegmentButtonIndex:)])
    {
        [self.delegate WK_StockChartSegmentView:self clickSegmentButtonIndex: btn.tag-WK_StockChartSegmentStartTag];
    }
}
